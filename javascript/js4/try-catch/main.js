...
const validacao = (nome) => {
  // valida se campo está vazio, caso não, dá um throw para ele cair no catch !
  if(nome === '') throw 'O campo está vazio'
  // valida se nome é do tipo string, senão dá um throw para elecair no catch
  if(typeof nome !== "string") throw 'Tipo inválida'
}

const procuraNome = nome => nomesNoRole.find((fulanoQueTavaNoRole, i) => {
  // procura o primeiro elemento no array que é verdadeiro de acordo com a operação lógica que você passar
...
const exibirNome = nome => {
  ...
  // caso estaNaLista não tiver valor, for vazia, ou undefined, da um throw para cair no catch
...
document.getElementById("form-do-role").addEventListener("submit", function(event){
  ...
  // precisamos validar o valor do valorCampo, caso esteja vazio ou não seja do tipo String
  // caso tudo der sucesso, imprime a mensage pro usuário, senão imprime o erro
  // para isso usaremos o try catch, try executa funções
  // caso dentro delas tenha um throw, irá cair no catch
  try{
