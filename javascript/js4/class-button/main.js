class Botao{
  // criamos uma classe porque precisamos criar um componente de botão, pois iremos reutilizá-lo varias vezes com cores e texto diferentes
  constructor(cor, tamanho, texto, icone){
    // fazemos o construtor para definir os atributos que a nossa classe terá
    this.cor = cor
  ...
  desenhaBotao(){
    // desenha botão é um método para inserir o botão na tela quando invocado
    const novoBotaoNoDom = document.createElement('button')
    // novoBotaoNoDom agora é uma tag button vazia e precisamos inserir o atributo class no html para colocar nossas classes css
    novoBotaoNoDom.setAttribute("class", `${this.cor} ${this.tamanho}`)
    // inserimos texto no html
    if(this.icone){
      //se this.icone tiver algum valor... crio uma nova tag <i></i> para poder inserir o ícone com fontawesome
...
const novoBotao = new Botao('verde', 'grande', 'botão')
console.log(novoBotao)
// desenhamos ele na tela
novoBotao.desenhaBotao()

const botaoRoxo = new Botao('roxo', 'pequeno', 'botão', 'fa-plus')
